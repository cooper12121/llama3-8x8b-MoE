...
class LlamaSparseMoeBlock(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        routing_weights, selected_experts = torch.topk(routing_weights, self.top_k, dim=-1)#[b*s,top_k],专家编号
        ...
        # if rank==0:
        # with open(f"/apdcephfs_qy3/share_301372554/share_info/qianggao/Yi/figure/CrimeKgAssistant/sft_coldboot_rank{rank}.txt", "a") as f:
        # # with open(f"/apdcephfs_qy3/share_301372554/share_info/qianggao/Yi/eval_router_warmboot/figure/codealpaca/sft_warmboot1.0.txt", "a") as f:
        #     if self.layer_idx==0:
        #         # row_title = ["layer_id"].append(f"expert_{str(x)}_nums" for x in range(self.num_experts)) #错 .append的返回值是None,改变的是原列表
                
        #         row_title = ["layer_id"]
        #         row_title.extend(f"expert_{str(x)}_nums" for x in range(self.num_experts)) 
        #         f.write(",".join([x for x in row_title]))
        #         f.write("\n")

        #     f.write(",".join([str(x) for x in choices]))
        #     f.write("\n")
...
class LlamaDecoderLayer(nn.Module):
    def __init__(self, config: LlamaConfig, layer_idx: int):
        ...
        self.block_sparse_moe = LlamaSparseMoeBlock(config,layer_idx=layer_idx)
...
class LlamaMoEForCausalLM(LlamaPreTrainedModel):
